-- PPU, Persentage of paying users
...
dau AS (
	SELECT CAST(sessions.begin_dttm AS DATE) AS dt, COUNT(DISTINCT sessions.user_id) AS count_user
	FROM sessions
	GROUP BY dt
),
pu AS (
	SELECT CAST(payments.payment_dttm AS DATE) AS dt, COUNT(DISTINCT payments.user_id) AS count_pay_user
	FROM payments
	GROUP BY dt
)
